...
static inline int32_t riscv_rem(int32_t a, int32_t b) {
    if (b == 0) return a;
    if (a == INT32_MIN && b == -1) return 0;
    return a % b;
}

static inline uint32_t riscv_remu(uint32_t a, uint32_t b) {
    if (b == 0) return a;
    return a % b;
}

static inline int32_t riscv_div(int32_t a, int32_t b) {
    if (b == 0) return -1;
    if (a == INT32_MIN && b == -1) return INT32_MIN;
    return a / b;
}

static inline uint32_t riscv_divu(uint32_t a, uint32_t b) {
